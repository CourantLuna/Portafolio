...
const skills = [
  { label: "Desarrollo Web", value: 90, color: "bg-pink-400" },
  { label: "Apps móviles", value: 50, color: "bg-violet-400" },
  { label: "Automatización / 3D", value: 85, color: "bg-blue-400" },
  { label: "UI/UX y prototipado", value: 80, color: "bg-pink-300" },
  { label: "Enseñanza STEAM", value: 95, color: "bg-blue-300" },
];
    ...
    <div className="w-full flex flex-col gap-10">
      ...
      <section className="grid md:grid-cols-2 gap-8 mb-10">
        ...
        <div>
          <div className="flex items-center gap-3 mb-5">
            ...
            <a
              ...
              className="group relative inline-flex items-center gap-1 px-3 py-1 bg-gradient-to-r from-pink-400 to-pink-500 text-white text-xs font-semibold rounded-full shadow-lg hover:shadow-pink-200 hover:shadow-lg transform hover:scale-105 transition-all duration-300 animate-pulse hover:animate-none"
            >
          ...
          <div className="flex flex-col gap-4">
            {education.map((ed, i) => (
              <div key={i} className={`rounded-xl p-4 ${ed.bg} shadow-sm h-32 flex flex-col items-start gap-1`}>
                <div className="text-xs text-gray-500 font-semibold">{ed.date}</div>
                <div className="font-bold text-lg text-gray-800">{ed.title}</div>
                <div className="text-gray-700">{ed.place}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-col gap-4">
            {experience.map((ex, i) => (
              <div key={i} className={`rounded-xl p-4 ${ex.bg} shadow-sm h-32 flex flex-col items-start gap-1`}>
                <div className="text-xs text-gray-500 font-semibold">{ex.date}</div>
                <div className="font-bold text-lg text-gray-800">{ex.title}</div>
                <div className="text-gray-700">{ex.place}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="grid md:grid-cols-2 gap-8 mb-10">
        {/* Skills */}
        <div>
          <h3 className="text-xl font-bold text-gray-900 mb-5">Habilidades</h3>
          <div className="flex flex-col gap-4">
            {skills.map((sk, i) => (
              <div key={i}>
                <div className="flex justify-between items-center">
                  <span className="font-medium text-gray-700">{sk.label}</span>
                  <span className="font-bold text-pink-400">{sk.value}%</span>
                </div>
                <div className="w-full h-2 rounded-full bg-gray-200 mt-1">
                  <div
                    className={`h-2 rounded-full ${sk.color} transition-all`}
        ...
        {/* Conocimientos */}
        <div>
          <h3 className="text-xl font-bold text-gray-900 mb-5">Conocimientos</h3>
